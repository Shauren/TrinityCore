...
{
    return i_mapEntry ? i_mapEntry->MapName[sWorld->GetDefaultDbcLocale()] : "UNNAMEDMAP";
}
...
{
    RemoveRespawnTimeDB(info, dbTrans);
...
void Map::RemoveRespawnTimeDB(RespawnInfo const* info, CharacterDatabaseTransaction dbTrans /*= nullptr*/)
{
    if (Instanceable())
        return;
...
InstanceMap::InstanceMap(uint32 id, time_t expiry, uint32 InstanceId, Difficulty SpawnMode, InstanceLock* instanceLock, Map* _parent)
...
{
    // TODO: worldstate management
    // set worldstate - this affects encounter journal display
    //if (updateSaveDataEvent.DungeonEncounter->CompleteWorldStateID)
    //    ;
