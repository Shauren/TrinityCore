...
{
    return _expiryTime < GameTime::GetGameTimeSystemPoint();
}
...
void InstanceLockMgr::Load()
{
    ...
    //                                                                 0     1                        2
    if (QueryResult result = CharacterDatabase.Query("SELECT instanceId, data, completedEncountersMask FROM instance2"))
    {
        do
        {
