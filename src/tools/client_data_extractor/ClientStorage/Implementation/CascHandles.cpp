...
namespace
{
    std::array<uint32, TOTAL_LOCALES> const WowLocaleToCascLocaleFlags =
    {
    ...
    char const* GetErrorText(uint32 error)
    {
        switch (error)
        {
    ...
    Optional<std::string> DownloadFile(std::string const& serverName, int16 port, std::string const& getCommand)
    {
        ...
        if (status_code != 200)
        {
            TC_LOG_ERROR("extractor.storage.casc", "Downloading tact key list failed with server response %u %s", status_code, status_message.c_str());
            return {};
        }
...
namespace CASC
{
char const* Storage::Name()
{
    return "CASC";
}

uint32 Storage::ConvertWowLocaleToStorageLocaleMask(LocaleConstant locale)
{
    ASSERT(locale < TOTAL_LOCALES);
    return WowLocaleToCascLocaleFlags[locale];
}

Storage::Storage(ClientStorage::StaticInterface const* staticInterface, HANDLE handle) : ClientStorage::Storage(staticInterface), _handle(handle)
{
}
...
ClientStorage::Storage* Storage::Open(boost::filesystem::path const& gameInstallPath, uint32 localeMask, char const* product)
{
    boost::filesystem::path const storagePath = (boost::filesystem::canonical(gameInstallPath) / "Data").make_preferred();
    std::string strPath = storagePath.string();
    ...
    if (!::CascOpenStorageEx(nullptr, &args, false, &handle))
    {
        ...
        TC_LOG_ERROR("extractor.storage.casc", "Error opening casc storage '%s': %s\n", storagePath.string().c_str(), GetErrorText(lastError));
        CascCloseStorage(handle);
        SetCascError(lastError);
        return nullptr;
    }

    TC_LOG_INFO("extractor.storage.casc", "Opened casc storage '%s'\n", storagePath.string().c_str());
    Storage* storage = new Storage(GetClientStorageStaticInterface(gameInstallPath), handle);

    if (!storage->LoadOnlineTactKeys())
        TC_LOG_ERROR("extractor.storage.casc", "Failed to load additional encryption keys from wow.tools, some files might not be extracted.\n");
...
ClientStorage::File* Storage::OpenFile(ClientStorage::FileId const& id, uint32 localeMask, bool printErrors /*= false*/, bool zerofillEncryptedParts /*= false*/) const
{
    DWORD openFlags = 0;
    if (zerofillEncryptedParts)
        openFlags |= CASC_OVERCOME_ENCRYPTED;

    void const* nameArg = nullptr;
    if (uint32 const* fileDataId = id)
    {
        openFlags |= CASC_OPEN_BY_FILEID;
        nameArg = CASC_FILE_DATA_ID(*fileDataId);
    }
    else if (std::string const* fileName = id)
    {
        openFlags |= CASC_OPEN_BY_NAME;
        nameArg = fileName->c_str();
    }

    HANDLE handle = nullptr;
    if (!::CascOpenFile(_handle, nameArg, localeMask, openFlags, &handle))
    {
        DWORD lastError = GetCascError(); // support checking error set by *Open* call, not the next *Close*
        if (printErrors)
            TC_LOG_ERROR("extractor.storage.casc", "Failed to open '%s' in CASC storage: %s", id.ToString().c_str(), GetErrorText(lastError));
...
char const* Storage::GetLastErrorText() const
{
    return GetErrorText(GetCascError());
}

uint32 Storage::NoneLocaleMask()
{
    return CASC_LOCALE_NONE;
}

uint32 Storage::AnyLocaleMask()
{
    return CASC_LOCALE_ALL_WOW;
}

bool Storage::Supports(boost::filesystem::path const& gameInstallPath)
{
    return boost::filesystem::exists(gameInstallPath / "Data" / "config")
        && boost::filesystem::exists(gameInstallPath / "Data" / "data")
        && boost::filesystem::exists(gameInstallPath / "Data" / "indices")
        && boost::filesystem::exists(gameInstallPath / ".build.info");
}
